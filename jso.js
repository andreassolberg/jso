;(function(exp, $) {
         ...
         */
        var uuid = function() {
            return 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, function uuidCallback(c) {
        ...
        var log = function(msg) {
            if (!options.debug || !console || !console.log) { return; }
        ...
        Api_default_storage.prototype.hasScope = function(token, scope) {
            var i = 0,
                len = token.scopes && token.scopes.length > 0 ? token.scopes.length : 0;
        ...
        exp.jso_checkfortoken = function(providerID, url, callback) {
            ...
            if (atoken.state) {
                ...
                if (!providerID) { throw new Error("Could not get [state] and no default providerid is provided."); }
            ...
            if (!config[state.providerID]) { throw new Error("Could not retrieve config for this provider."); }
        ...
        $.oajax = function(settings) {
            ...
            var performAjax = function() {
                if (!token) { throw new Error("Could not perform AJAX call because no valid tokens was found."); }
            ...
            return $.Deferred(function (dfd) {
                if (!token) {
                    if (!allowia) {
                        throw new Error("Could not perform AJAX call because no valid tokens was found.");
                    }

                    log("Perform authrequest");
                    jso_authrequest(providerid, scopes, function authTokenCallback() {
                        token = api_storage.getToken(providerid, scopes);
                        performAjax()
                            .done(dfd.resolve)
                            .fail(dfd.reject);
